...
bool DIEM::KiemTraHinhChuNhatHopLe(const DIEM& d1, const DIEM& d2) const
{
	// Kiểm tra chiều dài và chiều rộng có lớn hơn 0 hay không
	double dai = abs(d1.GetX() - x);
	double rong = abs(d1.GetY() - y);

	if (dai <= 0 || rong <= 0)
		return false;

	// Kiểm tra hai cạnh đường chéo có vuông góc với nhau hay không
	double d1d2 = (d1.GetX() - x) * (d2.GetX() - x) + (d1.GetY() - y) * (d2.GetY() - y);

	return abs(d1d2) < epsilon;
}

bool DIEM::KiemTraHinhChuNhatHopLe(double dai, double rong) const
{	
	return (dai > 0 && rong > 0);
}

double DIEM::TinhChuViHinhChuNhat(const DIEM& d1, const DIEM& d2) const
{	
	double dai = abs(d1.GetX() - x);
	double rong = abs(d1.GetY() - y);
...
double DIEM::TinhDienTichHinhChuNhat(const DIEM& d1, const DIEM& d2) const
{	
	double dai = abs(d1.GetX() - x);
	double rong = abs(d1.GetY() - y);
